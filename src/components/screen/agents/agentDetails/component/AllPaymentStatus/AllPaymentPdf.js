...
const AllPaymentPdf = ({ rowData = [], agentInfo = {} }) => {
  ...
  const totalPaid         = rowData.reduce((s, r) => s + (r.totalPaid    || 0), 0);
  const totalPending      = rowData.reduce((s, r) => s + (r.totalPending || 0), 0);
  const totalMembers      = new Set(rowData.map((r) => r.registrationNumber)).size;
  const totalPrograms     = new Set(rowData.map((r) => r.programName)).size;
  const totalPaidCount    = rowData.reduce((s, r) => s + (r.paidCount    || 0), 0);
  const totalPendingCount = rowData.reduce((s, r) => s + (r.pendingCount || 0), 0);
    ...
    <>
      ...
      <View style={styles.totalRow}>
        <View style={[styles.totalCell, { width: '85%' }]}>
          <Text style={[styles.totalLabel, { fontSize: 8 }]}>कुल बकाया राशि:</Text>
        </View>
        <View style={[styles.totalCell, { width: '15%', borderRightWidth: 0, backgroundColor: '#8B0000' }]}>
          <Text style={[styles.totalAmount, { fontSize: 9 }]}>{formatCurrency(totalPending)}</Text>
        </View>
      </View>
    </>
  );
